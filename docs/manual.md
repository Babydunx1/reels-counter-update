...
### 🎉 New Feature (facebook ระบบดีงวันที่)
- **JSON-First Date Extraction Engine**  
  - เพิ่มฟังก์ชัน `get_reel_date_via_json_driver(driver, url)` ที่ดึง UNIX timestamp จาก `<script type="application/json">` แล้วแปลงเป็นวันที่ (UTC+7)  
  - รองรับทั้ง `creation_time` และ field อื่นๆ ลงท้ายด้วย `time` ใน JSON
  - ถ้า JSON-First สำเร็จครบทุกคลิป → ข้ามขั้นตอน jump-scroll + scroll-loop ทั้งหมด
  - ถ้า JSON-First ล้มเหลวสำหรับแต่ละคลิป (เก็บใน `failed_indices`) → ค่อยสั่ง pre-scroll + scroll-loop แล้ว fallback ด้วยระบบเดิม (Smart Pre-Scroll & Stealth Date Fetch)
  - ลดการพึ่งพา XPath/text parsing ให้เหลือเฉพาะกรณี fallback
...
- 🧹 **Auto Close (ปิดเบราเซอร์เมื่อเริ่มค้นหาชุดใหม่)** เมื่อผู้ใช้กด "เริ่มใหม่" เพื่อดึงลิงก์ Reels ใหม่  
  → ระบบจะปิดเบราเซอร์ Standby ทันที เพื่อ:
  - 🔁 เริ่มรอบใหม่อย่างสะอาด  
  - ❌ ป้องกัน session เก่าค้างใน DOM  
  - 🧠 เคลียร์ RAM และ resource ที่ไม่จำเป็น  
  - ⚠️ ป้องกันปัญหา XPath ผิดจาก DOM เดิม  

### 🎨 ระบบสีสถานะเบราเซอร์ (Status Indicator)

ระบบแสดงสถานะของเบราเซอร์ Standby ด้วย "ไอคอนสี" ที่เข้าใจง่าย เพื่อ Debug ได้ด้วยตัวเอง:

| สีไอคอน | ความหมาย |
|--------|-----------|
| 🟢 สีเขียว | เบราเซอร์ Standby พร้อมใช้งาน |
| 🟠 สีส้ม | กำลังเปิด Standby และสแกนวันที่แบบ auto 1-4 คลิปแรก และ 1-4 คลิปสุดท้าย |
| 🔴 สีแดงกระพริบ | เบราเซอร์ถูกปิดโดยระบบ (อัตโนมัติ), ไม่มีเบราเซอร์ซ้อนหรือ process ค้างแน่นอน |
